...
robust=function (g, type = c("vertex", "edge"), measure = c("btwn.cent", 
                                                            "degree", "random"), N = 1000) 
{
  ...
  if (type == "vertex") {
    ...
    if (measure == "degree") {
      ...
      max.comp <- foreach(i = seq_len(N), .combine = "cbind") %dopar% 
        {
          el <- as_edgelist(g, names = FALSE)[index[, 
                                                    i], ]
          tmp <- rep.int(orig_max, n)
          for (j in seq_len(n - 1L)) {
            g.rand <- graph_from_edgelist(el[-seq_len(j), 
                                             , drop = FALSE], directed = FALSE)
      ...
      for (j in seq_len(n - 1L)) {
        g <- graph_from_edgelist(el[-seq_len(j), , drop = FALSE], 
                                 directed = FALSE)
        max.comp.removed[j + 1L] <- max(components(g)$csize)
      }
    }
...
###remove_vertex_randomly
random2=robust(igraph_s,"vertex",measure="random")

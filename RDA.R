...
head(df)
#环境因子数据
env <- read.csv("Dust_drug.csv",header = T,row.names = 1,check.names = F)
head(env)
print(decorana(t(df)))
#根据DCA1的Axis Lengths值进行选择，如果>4.0选CCA；如果在3.0-4.0之间，选RDA和CCA都可以；如果<3.0, 选择RDA分析即可
RDA <- rda(t(df),env,scale = T)
#db-RDA
dbRDA <- dbrda(df~.,env,dist="bray")
test=anova(dbRDA,by="terms", permu=999)
 #查看结果，把P值<0.05的环境因子筛选出来#
envfit(dbRDA,env,permutations=999)
dbRDA=dbrda(t(df)~Citalopram + Amitriptyline + Benzyl_paraben + Propyl_paraben + Codeine + Cetirizine + Ethyl_paraben + Methyl_paraben + Gabapentin + Roxithromycin + Caffeine + Desvenlafaxin + Fluconazole + Trimethoprim,data = env)   
vif.cca(RDA) #查看，若出现>20（不相关）的环境因子，需要剔除；若超过3个，需逐步从大到小依次剔除#
anova(dbRDA, by="term",permutations = 999) #ANOVA 显著性分析
permutest(dbRDA,permutations = 999)
#提取数据
df_rda <- data.frame(dbRDA$CCA$u[,1:2],rownames(env))
colnames(df_rda)=c("dbRDA1","dbRDA2","samples")
# 提取物种得分
df_rda_score <- data.frame(dbRDA$CCA$v[,1:2])
#计算轴标签数据（=轴特征值/sum(所有轴的特征值)）
dbRDA1 =round(dbRDA$CCA$eig[1]/sum(dbRDA$CCA$eig)*100,2)
dbRDA2 =round(dbRDA$CCA$eig[2]/sum(dbRDA$CCA$eig)*100,2)
...
#提取环境因子得分
df_rda_env <- dbRDA$CCA$biplot[,1:2]
...
p1+
  geom_segment(data=df_rda_env,aes(x=0,y=0,xend=df_rda_env[,1],yend=df_rda_env[,2]),
               ...
               arrow=arrow(angle = 35,length=unit(0.1,"cm")))+
